...
OnDraw( function ()
	...
	for _,i in pairs(obj) do
		...
		if i.safe then DrawCircle(i.safe,i.spell.radius*.5,0,3,GoS.Green) end
	end
end)

OnProcessSpell( function(unit,spellProc)
	if spellProc.target == myHero and s[GetObjectName(unit)] then
		for _,i in pairs(s[GetObjectName(unit)]) do
			if i.spellName == spellProc.name then
				--CastTargetSpell(myHero,2) --KarmaRama
			end
		end
	end
end)
...
OnTick( function()
	for _,i in pairs(obj) do
		...
		S1 = GetOrigin(myHero)+Vector(i.sPos-i.ePos):perpendicular()
		S2 = GetOrigin(myHero)
		jp = Vector(VectorIntersection(i.sPos,i.ePos,S1,S2).x,i.ePos.y,VectorIntersection(i.sPos,i.ePos,S1,S2).y)
		i.jp = jp
		i.safe = GetOrigin(myHero) + Vector(i.sPos-i.ePos):perpendicular()
		if GetDistance(GetOrigin(myHero) + Vector(i.sPos-i.ePos):perpendicular(),jp) >= GetDistance(GetOrigin(myHero) + Vector(i.sPos-i.ePos):perpendicular2(),jp) then
			safe = GetOrigin(myHero) + Vector(i.sPos-i.ePos):perpendicular()
		else
			safe = GetOrigin(myHero) + Vector(i.sPos-i.ePos):perpendicular2()
		end
		if GetDistance(myHero,jp) < i.spell.radius*3 and GetDistance(myHero,i.Obj) < GetHitBox(myHero)+i.spell.speed*.75 and GetDistance(myHero,i.Obj) < GetHitBox(myHero)+i.spell.speed*.2 then
			CastSkillShot(2,safe)
		end
	end
end)
